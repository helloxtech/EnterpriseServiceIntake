...
| --- | --- | --- | --- |
| UX-F01 | High | Power Pages Web API site settings expose `fields: '*'` and `Webapi/error/innererror=true`. This is broader than required for the external UX and conflicts with the intent to hide internal fields/errors. | Replace `fields: '*'` with explicit allowlists and disable `Webapi/error/innererror` outside debugging. |
